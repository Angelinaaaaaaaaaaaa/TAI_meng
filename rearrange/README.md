...
│
├── src/
│   ├── pipeline.py                       (top-level orchestration + CLI)        
│   ├── core/
│   │   └── steps.py                      (domain logic: pre-merge, enrich,    
│   │                                      backbone, orphans, plan)
│   ├── services/
│   │   ├── llm_gateway.py                (OpenAI structured-completion wrapper) 
│   │   └── models.py                     (Pydantic schemas + PipelineContext)   
│   └── utils/
│       └── utils.py                      (logging, paths, JSON I/O, ContextVar) 
│
...
The  project-root `__init__.py` makes `rearrange` a package, and all in-tree
