...
├── steps.py           # Domain logic — pre-merge → enrich → backbone →     (~1380 LoC)
│                      #   orphans → plan
├── pipeline.py        # Orchestration, tree builder, CLI entry              (~850 LoC)
├── file_rearrang.py   # Back-compat re-export shim                          (~95 LoC)
│
